    ...
    yield* Effect.promise(async () => {
      ...
      async function execute(sdk: OpencodeClient) {
        ...
        async function loop(client: OpencodeClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
          ...
          // Live-streaming: the server also emits "message.part.delta" events
          // (raw incremental text, distinct from "message.part.updated" which
          // only fires once at part-start (empty) and once at part-end (full
          // text)) -- print those as they arrive instead of waiting for the
          // part to finish. partKind remembers which in-flight part IDs are
          // text/reasoning so a delta event (which only carries a partID, not
          // a part type) knows how to handle it; streamedPartIDs remembers
          // which parts we've already streamed so the part-end handler below
          // doesn't reprint the whole accumulated text a second time.
          const partKind = new Map<string, "text" | "reasoning">()
          ...
          for await (const event of events.stream) {
            ...
            if (event.type === "message.part.delta") {
              ...
              if (emit("part_delta", { properties: d })) continue
              if (d.field !== "text") continue
              const kind = partKind.get(d.partID)
              if (!kind) continue
              if (kind === "reasoning" && !thinking) continue
              if (kind === "reasoning" && !streamedPartIDs.has(d.partID) && process.stdout.isTTY) {
                process.stdout.write(`${UI.Style.TEXT_DIM}\u001b[3m`)
              }
              streamedPartIDs.add(d.partID)
            ...
            if (event.type === "message.part.updated") {
              ...
              if (part.type === "text" && args.format !== "json") {
                if (!part.time?.end) {
                  partKind.set(part.id, "text")
                } else if (streamedPartIDs.has(part.id)) {
                  streamedPartIDs.delete(part.id)
                  partKind.delete(part.id)
                  if (process.stdout.isTTY) UI.empty()
                  else process.stdout.write(EOL)
                  continue
                }
              }
              ...
              if (part.type === "reasoning" && thinking && args.format !== "json") {
                if (!part.time?.end) {
                  partKind.set(part.id, "reasoning")
                } else if (streamedPartIDs.has(part.id)) {
                  streamedPartIDs.delete(part.id)
                  partKind.delete(part.id)
                  process.stdout.write((process.stdout.isTTY ? `\u001b[0m${UI.Style.TEXT_NORMAL}` : "") + EOL)
                  if (process.stdout.isTTY) UI.empty()
                  continue
                }
              }
