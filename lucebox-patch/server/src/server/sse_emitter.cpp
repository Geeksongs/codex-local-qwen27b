...
// Responses API "custom" tools (freeform / grammar-constrained, e.g. Codex's
// apply_patch) use a distinct wire item type (`custom_tool_call` with an
// `input` string field) instead of the regular `function_call` item
// (`arguments` JSON string). Look up how the request declared this tool.
static bool is_custom_tool(const json & tools, const std::string & name) {
    if (!tools.is_array()) return false;
    for (const auto & t : tools) {
        if (t.is_object() && t.value("name", "") == name) {
            return t.value("type", "") == "custom";
        }
    }
    return false;
}
...
SseEmitter::SseEmitter(ApiFormat format,
    ...
    , started_in_thinking_(started_in_thinking)
{
...
std::vector<std::string> SseEmitter::emit_start() {
    ...
    case ApiFormat::RESPONSES: {
        ...
        // When the request starts inside <think>, defer the message item:
        // the reasoning item opens lazily in emit_reasoning_delta() (so its
        // own output_item.added fires at output_index 0), and the message
        // item opens lazily in emit_content_delta() once the model leaves
        // <think>, at whatever output_index follows.
        if (!started_in_thinking_) {
            // output_item.added
            out.push_back(format_responses_event("response.output_item.added", {
                {"output_index", responses_output_index_},
            ...
            out.push_back(format_responses_event("response.content_part.added", {
                {"item_id", msg_item_id_}, {"output_index", responses_output_index_},
                {"content_index", 0},
                {"part", {{"type", "output_text"}, {"text", ""}, {"annotations", json::array()}}}
            }));
            responses_msg_item_open_ = true;
        }
        break;
    }
    ...
    while (true) {
        ...
        if (mode_ == StreamMode::REASONING) {
            ...
            if (tool_hit) {
                ...
                close_responses_reasoning_item(out);
...
void SseEmitter::emit_reasoning_delta(std::vector<std::string> & out,
    ...
    case ApiFormat::RESPONSES:
        if (!responses_reasoning_open_) {
            reasoning_item_id_ = gen_item_id();
            out.push_back(format_responses_event("response.output_item.added", {
                {"output_index", responses_output_index_},
                {"item", {{"type", "reasoning"}, {"id", reasoning_item_id_},
                          {"status", "in_progress"}, {"summary", json::array()}}}
            }));
            out.push_back(format_responses_event("response.reasoning_summary_part.added", {
                {"item_id", reasoning_item_id_}, {"output_index", responses_output_index_},
                {"summary_index", 0},
                {"part", {{"type", "summary_text"}, {"text", ""}}}
            }));
            responses_reasoning_open_ = true;
        }
        out.push_back(format_responses_event("response.reasoning_summary_text.delta", {
            {"item_id", reasoning_item_id_}, {"output_index", responses_output_index_},
...
void SseEmitter::close_responses_reasoning_item(std::vector<std::string> & out) {
    if (!responses_reasoning_open_) return;
    out.push_back(format_responses_event("response.reasoning_summary_text.done", {
        {"item_id", reasoning_item_id_}, {"output_index", responses_output_index_},
        {"summary_index", 0}, {"text", reasoning_text_}
    }));
    out.push_back(format_responses_event("response.reasoning_summary_part.done", {
        {"item_id", reasoning_item_id_}, {"output_index", responses_output_index_},
        {"summary_index", 0},
        {"part", {{"type", "summary_text"}, {"text", reasoning_text_}}}
    }));
    out.push_back(format_responses_event("response.output_item.done", {
        {"output_index", responses_output_index_},
        {"item", {{"type", "reasoning"}, {"id", reasoning_item_id_},
                  {"status", "completed"},
                  {"summary", json::array({{{"type", "summary_text"}, {"text", reasoning_text_}}})}}}
    }));
    responses_reasoning_open_ = false;
    responses_output_index_++;
}
...
std::vector<std::string> SseEmitter::emit_finish(int completion_tokens,
    ...
    // Generation ended (EOS, stop sequence, or token cap) — close out any
    // still-open Responses API reasoning item so response.completed always
    // sees a fully closed item list, even if the model never naturally
    // left <think> (e.g. hit max_tokens while still reasoning).
    close_responses_reasoning_item(out);
    ...
    if (mode_ == StreamMode::TOOL_BUFFER && !tool_buffer_.empty()) {
        ...
        if (!tool_calls_.empty()) {
            ...
            case ApiFormat::RESPONSES:
                for (const auto & tc : tool_calls_) {
                    if (is_custom_tool(tools_, tc.name)) {
                        out.push_back(format_responses_event(
                            "response.custom_tool_call_input.delta", {
                                {"item_id", tc.id}, {"output_index", responses_output_index_},
                                {"delta", tc.arguments}
                            }));
                        json done_ev = {
                            {"item_id", tc.id}, {"output_index", responses_output_index_},
                        ...
                        if (!tc.namespace_.empty()) done_ev["namespace"] = tc.namespace_;
                        out.push_back(format_responses_event(
                            "response.custom_tool_call_input.done", done_ev));
                    } else {
                        out.push_back(format_responses_event(
                        ...
                        if (!tc.namespace_.empty()) done_ev["namespace"] = tc.namespace_;
                        out.push_back(format_responses_event(
                            "response.function_call_arguments.done", done_ev));
                    }
                }
                break;
            default: break;
    ...
    case ApiFormat::RESPONSES: {
        // output_text.done / content_part.done only apply to a message item
        // that was actually opened — skipped for a response that was pure
        // reasoning + tool calls with no visible answer text.
        if (responses_msg_item_open_) {
        ...
        // Build final output items. A reasoning item (if the model streamed
        // one) always sits at output_index 0, before whatever came next.
        json final_output = json::array();
        if (!reasoning_text_.empty()) {
            final_output.push_back({
                {"type", "reasoning"}, {"id", reasoning_item_id_},
                {"status", "completed"},
                {"summary", json::array({{{"type", "summary_text"}, {"text", reasoning_text_}}})}
            });
        }
        if (!tool_calls_.empty()) {
            for (const auto & tc : tool_calls_) {
                if (is_custom_tool(tools_, tc.name)) {
                    json item = {
                        {"type", "custom_tool_call"}, {"id", tc.id},
                        {"status", "completed"}, {"call_id", tc.id},
                        {"name", tc.name}, {"input", tc.arguments}
                    };
                    if (!tc.namespace_.empty()) item["namespace"] = tc.namespace_;
        ...
        } else if (responses_msg_item_open_) {
            final_output.push_back({
        ...
        // output_item.done for each item. The reasoning item's own
        // output_item.done already streamed from close_responses_reasoning_item()
        // above, so start numbering the rest from its output_index + 1 (or 0
        // if there was no reasoning at all).
        {
            int base = reasoning_text_.empty() ? 0 : 1;
            for (size_t i = 0; i + base < (size_t)final_output.size(); i++) {
                out.push_back(format_responses_event("response.output_item.done", {
                    {"output_index", (int)(base + i)},
                    {"item", final_output[base + i]}
                }));
            }
        }
