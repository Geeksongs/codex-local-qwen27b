...
struct ToolCall {
    ...
    // Set when `name` is a sub-tool of a Responses API "namespace" tool
    // (e.g. Codex's multi_agent_v1, which bundles spawn_agent/wait_agent/
    // etc.). Codex's wire protocol carries the namespace as a field
    // separate from the tool name on the function_call/custom_tool_call
    // item — confirmed by reading its (open-source) request dispatcher,
    // not guessed — so this must round-trip back out alongside `name`,
