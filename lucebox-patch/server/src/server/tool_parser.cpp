...
// Check if a function name is in the allowed tools list. Handles three
// Responses API tool shapes: plain `{"name": ...}`, chat-completions-style
// `{"function": {"name": ...}}`, and `{"type": "namespace", "tools": [...]}`
// (e.g. Codex's multi_agent_v1) — a namespace tool bundles several real,
// independently-callable sub-tools (spawn_agent, wait_agent, ...) under one
// grouping entry; the model is meant to call a sub-tool's own name directly,
// so those names must be checked too, not just the namespace's own name.
static bool tool_allowed(const json & tools, const std::string & name) {
    ...
    for (const auto & t : tools) {
        ...
        if (t.value("type", "") == "namespace" && t.contains("tools") &&
            t["tools"].is_array()) {
            for (const auto & sub : t["tools"]) {
                const auto & subfn = sub.contains("function") ? sub["function"] : sub;
                if (subfn.is_object() && subfn.value("name", "") == name) return true;
            }
        }
    }
    return false;
...
// If `name` is a sub-tool bundled inside a `{"type": "namespace", ...}`
// entry, return that entry's own `name` (e.g. "multi_agent_v1" for
// "spawn_agent"). Empty string if `name` is a plain top-level tool.
static std::string find_tool_namespace(const json & tools, const std::string & name) {
    ...
    for (const auto & t : tools) {
        ...
        for (const auto & sub : t["tools"]) {
            const auto & subfn = sub.contains("function") ? sub["function"] : sub;
            if (subfn.is_object() && subfn.value("name", "") == name) {
                return t.value("name", "");
            }
        }
    }
...
static json properties_from_def(const json & fn) {
...
static json find_tool_properties(const json & tools, const std::string & name) {
    ...
    for (const auto & t : tools) {
        ...
        if (fn.is_object() && fn.value("name", "") == name) {
            json props = properties_from_def(fn);
        ...
        if (t.value("type", "") == "namespace" && t.contains("tools") &&
            ...
            for (const auto & sub : t["tools"]) {
                const auto & subfn = sub.contains("function") ? sub["function"] : sub;
                if (subfn.is_object() && subfn.value("name", "") == name) {
                    json props = properties_from_def(subfn);
                    if (!props.empty()) return props;
                }
            }
        }
...
ToolParseResult parse_tool_calls(const std::string & text, const json & tools) {
    ...
    auto add_call = [&](const std::string & fn_name, const json & args,
        ...
        tc.name = fn_name;
        tc.namespace_ = find_tool_namespace(tools, fn_name);
        tc.arguments = raw_args.empty() ? args.dump() : raw_args;

        // Codex's `apply_patch` is a "freeform" tool: its wire contract is a
        // single raw string (the patch text itself), not a JSON object. Every
        // model we've seen was fine-tuned to always emit named JSON
        // parameters for tool calls regardless of system-prompt instructions
        // otherwise, so it wraps the patch text as e.g. {"patch": "..."} —
        // valid JSON, but "invoked with incompatible payload" to a freeform
        // tool. Unwrap that single string value here so the arguments we
        // forward match what a freeform tool caller expects, independent of
        // whatever key name the model chose.
        if (fn_name == "apply_patch" && raw_args.empty() &&
            args.is_object() && args.size() == 1) {
            auto it = args.begin();
            if (it->is_string()) {
                tc.arguments = it->get<std::string>();
            }
        }
