...
private:
    ...
    // Responses API only: close the in-progress reasoning item (if one was
    // opened by emit_reasoning_delta) with its done/summary/output_item
    // events. Idempotent — a no-op once the item is closed or if reasoning
    // was never streamed for this request.
    void close_responses_reasoning_item(std::vector<std::string> & out);
    ...
    // Responses API: lazy reasoning/message item lifecycle. When the
    // request starts inside <think>, the reasoning item (and its
    // reasoning_summary_text.delta stream) opens on the first
    // emit_reasoning_delta() call instead of at emit_start(), so the
    // client sees live progress during the thinking phase instead of
    // total silence until the final answer is ready. The message item
    // then opens lazily — at whatever output_index follows — once the
    // model transitions out of <think>.
    bool         started_in_thinking_ = false;
    bool         responses_reasoning_open_ = false;
    bool         responses_msg_item_open_ = false;
    int          responses_output_index_ = 0;
    std::string  reasoning_item_id_;
