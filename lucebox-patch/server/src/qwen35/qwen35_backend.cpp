...
GenerateResult Qwen35Backend::generate_impl(const GenerateRequest & req,
    ...
    // KVFlash live-pool continuation. Once a session's cumulative context
    // has ever exceeded the resident pool, snapshot_save() permanently
    // refuses to save (cache_.cur_pos > kvflash_tokens_, see the guard
    // there), so http_server.cpp can never again find a restore slot for
    // it and always calls this plain generate() with the FULL cumulative
    // prompt. Reprefilling that whole prompt from scratch every turn is
    // the real cost behind "every round-trip costs ~24s once a long
    // session crosses the pool boundary" -- but nothing tore the live
    // cache down between requests (single FIFO worker; no other session
    // interleaved), so if this prompt is exactly the previous one plus a
    // new suffix, the pool already holds everything up to cache_.cur_pos
    // and only the suffix needs prefilling. kvflash_history_ is the
    // host-side record of exactly which token sits at each resident
    // logical position, so a byte-exact prefix match against it is a
    // sufficient, cheap correctness check (identical in spirit to the
    // prefix match http_server.cpp already trusts before restoring from
    // an explicit snapshot slot -- this just skips the snapshot's host
    // round-trip because the data was never evicted from the live cache).
    bool kv_pool_continue = false;
    int  continue_kv_offset = 0;
    if (!cfg_.paged_attention && kvflash_active() &&
        kvflash_prompt_boundary_pos_ > 0 &&
        kvflash_history_.size() >= (size_t)kvflash_prompt_boundary_pos_ &&
        req.prompt.size() >= 1) {
        // Longest common prefix against the last known-good prompt
        // boundary, not a strict all-or-nothing match: chat templates
        // commonly retokenize a few tokens differently right at a message
        // boundary once trailing content changes from "prompt awaiting a
        // reply" to "closed message followed by more content" (BPE merge
        // effects), so requiring the ENTIRE boundary to match byte-exact
        // would almost always fail on exactly the tokens that matter
        // least. Any tokens that genuinely still match get reused; the
        // rest (even if that's everything) is simply reprocessed as the
        // delta, which is always correct, just less of a speedup.
        const int limit = std::min(kvflash_prompt_boundary_pos_, (int)req.prompt.size());
        int lcp = 0;
        while (lcp < limit && kvflash_history_[lcp] == req.prompt[lcp]) lcp++;
        if (lcp > 0) {
            kv_pool_continue   = true;
            continue_kv_offset = lcp;
        }
    }
    ...
    // position-addressed and will be overwritten during prefill. Skipped
    // for a live continuation: the recurrent state already correctly
    // reflects everything up to cache_.cur_pos and was never reset.
    if (!kv_pool_continue) {
    ...
    if (kv_pool_continue) {
        ...
        if (prompt_len > continue_kv_offset) {
            std::vector<int32_t> delta(req.prompt.begin() + continue_kv_offset,
                                       req.prompt.end());
            committed = do_prefill(delta, out_io, req.snap_pos, req.snap_slot,
                                   /*kv_offset=*/continue_kv_offset,
                                   /*kv_pool_continue=*/true);
        } else {
            // Exact match, nothing new to prefill: no per-step graph was
            // built this request, so build the single-token decode step
            // graph a real prefill would otherwise have left behind
            // (mirrors restore_and_generate_impl's exact-hit branch).
            committed = continue_kv_offset;
        ...
        if (committed >= 0) {
            result.restored_prefix_tokens = continue_kv_offset;
        }
    } else {
...
int Qwen35Backend::do_prefill(const std::vector<int32_t> & tokens,
    ...
    if (kvf_paged && kv_offset != 0 && !kv_pool_continue) {
        std::fprintf(stderr,
    ...
    if (kvf_paged) {
        ...
        if (kv_pool_continue) {
            // Appending onto the pool's own still-live state: chunks_ /
            // free_blocks_ already correctly describe [0, kv_offset)
            // exactly as if slot_for() had been called incrementally by a
            // single unbroken request, because it was — across separate
            // HTTP calls on the same backend. Do not reset; slot_for()
            // below extends the mapping and evicts LRU chunks exactly as
            // it already does mid-request.
            std::printf("[kvflash] pooled prefill: continuing live pool at "
                        "kv_offset=%d, +%d new tokens (%d-token pool, "
                        "%d-token chunks)\n",
    ...
    if (kvflash_active()) {
        if (kvf_paged) {
            ...
            if (kv_pool_continue) {
                // history[0, kv_offset) is already correct (nothing was
                // reset) — truncate any stale tail and append the new
                // tokens actually committed this call, rather than
                // discarding the whole known history like the fresh-start
                // branch below does.
    ...
    // Record the end of THIS prefill as the live-pool continuation
    // boundary (see kvflash_prompt_boundary_pos_'s declaration): the next
    // generate_impl() call can trust kvflash_history_[0, committed) as
    // genuine prompt content up to here, regardless of whatever this
    // backend generates afterward.
    if (kvflash_active()) {
        kvflash_prompt_boundary_pos_ = committed;
    }
