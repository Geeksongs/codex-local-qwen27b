...
protected:
    ...
    int  kvflash_tau_    = 64;
    // Logical position where the LAST prefill (not decode) ended -- i.e.
    // the length of the client-supplied prompt that was actually prefilled,
    // before any tokens this backend generated itself got appended. Used
    // to detect a live-pool continuation in generate_impl(): the client's
    // next prompt is compared against kvflash_history_[0, boundary) rather
    // than against [0, cache_.cur_pos), because cache_.cur_pos also
    // includes this backend's own raw generated token ids, which are not
    // guaranteed to retokenize identically once the client's chat template
    // re-renders them as message content in the next prompt. Matching only
    // up to the last genuine prompt boundary sidesteps that entirely: any
    // divergence (including the model's own last reply) is simply
    // reprocessed as part of the delta, exactly as the explicit
    // snapshot-restore path already does.
    int  kvflash_prompt_boundary_pos_ = 0;
...
private:
    ...
    // kv_pool_continue: true only when kv_offset == cache_.cur_pos AND the
    // KVFlash pager's existing chunk/block bookkeeping for [0, kv_offset)
    // is still live and valid (i.e. this call is appending directly onto
    // the SAME resident pooled state left by the previous request on this
    // backend, not restoring a separate snapshot into a freshly-reset
    // pool). It skips kvflash_pager_.reset() and the "restore offsets
    // unsupported in the pooled path" guard, letting slot_for() extend the
    // existing mapping (with ordinary LRU eviction) instead of discarding
    // it and reprefilling from scratch. Never set this for a snapshot
    // restore: the pager's bookkeeping there was not derived from prior
    // slot_for() calls and reset() is required for correctness.
